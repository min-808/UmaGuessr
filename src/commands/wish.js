            ...
            try {
                ...
                if (multipleWish == "10") {
                    ...
                    if (currentAmount < 1600) {
                        ...
                        await client.close()
                            ...
                            if (cooldowns.has(interaction.user.id)) { // But you're on cooldown
                                ...
                                setTimeout(() => cooldowns.delete(interaction.user.id), 8000);
                    ...
                    if (currentAmount < 160) {
                        ...
                        if (oneCooldowns.has(interaction.user.id)) { // But you're on cooldown
                            testEmbed.spliceFields(0, 1, {
                                name: "\n",
                                value: `Please wait a few seconds before doing another wish!`
                            })
    
                            interaction.editReply({ embeds: [testEmbed] });
                            await client.close()
                        } else { // Add to cooldowns, wait 1.5 seconds then remove from set
                            oneCooldowns.add(interaction.user.id)
                            setTimeout(() => oneCooldowns.delete(interaction.user.id), 1500);
...
You now have **${(await ids.findOne({discord_id: discordID}, options))['jade_count']}** stellar jade`
                ...
                interaction.editReply({ content: "Failed creating embed, please don't spam the command!"})
                await client.close()
            }
        })()
    }
